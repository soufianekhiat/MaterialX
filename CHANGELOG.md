...
### Added
- Added support for the HLSL shading language in MaterialX shader generation and rendering, with new `MaterialXGenHlsl` and `MaterialXRenderHlsl` modules targeting DirectX Shader Compiler (DXC) and Shader Model 5.x / 6.x. Generated shaders carry HLSL semantics (`SV_Position`, `TEXCOORD`, etc.), HLSL `cbuffer` uniforms, and HLSL-correct struct initialization (brace init for user struct types). An optional `HlslResourceBindingContext` emits explicit `register(b#)` annotations and reserves `t#`/`s#` slots for SamplerTexture2D handles. The renderer wraps both FXC (`d3dcompiler_47`, DXBC, SM 5.x) and DXC (`dxcompiler`, DXIL, SM 6.x with HLSL 2021) behind a single `HlslProgram` API, exposes D3D shader reflection (`ID3D11ShaderReflection` for DXBC and `ID3D12ShaderReflection` for DXIL) so callers can inspect the slot assignments the toolchain actually produced, and ships a full off-screen D3D11 rendering pipeline: `HlslContext` (device + WARP fallback), `HlslFramebuffer` (off-screen RT + readback), `HlslMaterial` (per-stage cbuffer pools with CPU-mirror partial writes, reflected member-offset lookup, texture/sampler binding), `HlslTextureHandler` (Image → SRV + sampler with caching), and `HlslRenderer` (a `ShaderRenderer` subclass that auto-binds camera matrices, mesh geometry from a `GeometryHandler`, file textures from an `ImageHandler`, env radiance/irradiance and per-light parameters from a `LightHandler`). Includes a `genhlsl` target definition under `libraries/targets`, per-library implementation overrides under `libraries/{stdlib,pbrlib,nprlib,lights}/genhlsl`, HLSL-specific math and texture helpers, language bindings for Python (`PyMaterialXGenHlsl`, `PyMaterialXRenderHlsl`) and JavaScript (`JsMaterialXGenHlsl`), and a Catch2 test suite that runs every standard test material through both the generator and D3DCompile end-to-end.
